...
(async function () {
  ...
  let dropdown2 = document.querySelectorAll('div[data-value="any_status"]')
  triggerMostButtons(dropdown2[1])
  ...
  async function collectLeads() {
    ...
    let table = document.querySelector('table.i3WFpf')
    console.log(table)
  ...
  await collectLeads()
  paginate.scrollIntoView(true)

  let looping = false;

  while (!looping) {

    let paginate2 = document.querySelectorAll('span[data-paginate="next"]');

    console.log("Pressing button")
    triggerMostButtons(paginate2[0])

    await waitSeconds(4)
    await collectLeads()

    let paginate3 = document.querySelectorAll('span[data-paginate="next"]');
    
    looping = (paginate3[0].ariaDisabled === "true") ? true : false
